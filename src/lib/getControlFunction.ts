type CalculateFunction = (value: number) => string | null;
interface ControlFunctionMap {
	[ccNumber: number]: string | CalculateFunction | undefined;
}

const controlOnOrOff = (paramName: string): CalculateFunction =>
	value => value < 64
		? `${paramName}off`
		: `${paramName}on`;

const controlValueMap = (paramMap: {[value: number]: string}): CalculateFunction =>
	value => paramMap[value] || null;

const controlFunctions: ControlFunctionMap = {
